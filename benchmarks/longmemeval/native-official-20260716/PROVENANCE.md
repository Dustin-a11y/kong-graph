...
2. Download the cleaned dataset (NOT included in the repo clone):
   ```bash
   mkdir -p LongMemEval/data
   wget -O LongMemEval/data/longmemeval_s_cleaned.json \
     https://huggingface.co/datasets/xiaowu0162/longmemeval-cleaned/resolve/main/longmemeval_s_cleaned.json
   ```

3. Create the credential file (template provided, NEVER commit with real keys):
   ```bash
   cp benchd-openrouter.env.example benchd-openrouter.env
   # Edit benchd-openrouter.env and set your OPENROUTER_API_KEY
   ```
   Both `evaluate_qa_openrouter.py` and `run_generation_openrouter.py` read this file.
   - `evaluate_qa_openrouter.py`: hardcoded path `./benchd-openrouter.env` (cannot override via CLI)
   - `run_generation_openrouter.py`: passed via `--cred-file ./benchd-openrouter.env`

4. Run retrieval (standalone; does NOT import kong-graph — only needs sentence-transformers, rank_bm25, torch, numpy):
   ```bash
...
5. Run generation (requires OpenRouter API key):
   ```bash
...
6. Run evaluation (requires OpenRouter API key; hardcoded to read `./benchd-openrouter.env`):
   ```bash
