"""
Full pipeline test — store memories, build graph, recall with greedy subgraph.
Also runs MemCombine benchmark comparing graph+greedy vs plain Top-K.
...
import numpy as np
from kong_graph.graph import MemoryGraph
from kong_graph.pipeline import store, recall, store_batch, set_graph, get_graph
from kong_graph.subgraph_optimizer import optimize_subgraph
...
def test_graph_construction():
    ...
    memories = [
        "The memory graph API runs on port 8501 on the DGX Spark server.",
        "Hardware cron runs 5 times daily using optimized backend scheduling.",
        "Chef decided to open source the memory graph project under MIT license.",
        "Copyright was updated to Coinkong Chef's Attraction for all source files.",
        "Team had pizza for lunch. The pepperoni was excellent.",
        "Greedy selection provides >93% optimality on memory selection problems.",
        ...
        "Sentence transformers power the embedding pipeline for retrieval.",
    ]
...
def test_neighborhood_search(g):
    ...
    # Query about Spark server infrastructure
    ...
    if pizza_scores:
        spark_scores = [score for mid, score in neighborhood.items()
                         if "spark" in g.memories[mid].text.lower() or "server" in g.memories[mid].text.lower()]
        if spark_scores:
            assert max(pizza_scores) < max(spark_scores), "Pizza should rank below infrastructure memories"

    print("  ✅ PASS\n")


def test_subgraph_optimizer():
    """Test greedy subgraph selection vs brute force."""
    print("=" * 60)
    print("TEST 3: Greedy Subgraph Optimizer")
    print("=" * 60)
    ...
    print(f"  Greedy selected: {result['selection']} (score: {result['score']:.4f})")
    print(f"  Greedy vs Optimal: {result['qaoa_vs_optimal_pct']:.1f}%")

    # Greedy should select the connected cluster (0,1,2) because
    ...
    assert result['qaoa_vs_optimal_pct'] >= 80, f"Greedy should be >=80% of optimal, got {result['qaoa_vs_optimal_pct']:.1f}%"
...
def test_full_pipeline():
    ...
    print(f"  Subgraph score: {recall_result.get('subgraph_score', '?')}")
...
def test_memcombine_benchmark():
    """Run MemCombine benchmark: Graph+Greedy vs Top-K."""
    print("=" * 60)
    ...
    def graph_recall(memories, query, K):
        """Our system: Graph + greedy subgraph."""
        g = MemoryGraph(similarity_threshold=0.15)
        set_graph(g)
        g.add_memories_batch(memories)
    ...
    print("\n  --- Graph + Greedy ---")
    greedy_results = run_benchmark(graph_recall, K=5)
    print(f"  Coverage:        {greedy_results['avg_coverage']*100:.1f}%")
    print(f"  Evidence Recall: {greedy_results['avg_evidence_recall']*100:.1f}%")
    print(f"  F1:              {greedy_results['avg_f1']*100:.1f}%")
    print(f"  Perfect:         {greedy_results['perfect_coverage']}/{greedy_results['n_scenarios']}")

    advantage = greedy_results['avg_coverage'] - topk_results['avg_coverage']
    print(f"\n  Graph Advantage (Coverage): {advantage*100:+.1f}%")

    # Print per-scenario comparison
    print("\n  Per-scenario breakdown:")
    for tk, ga in zip(topk_results['per_scenario'], greedy_results['per_scenario']):
        icon = "🟢" if ga['coverage'] > tk['coverage'] else ("🟡" if ga['coverage'] == tk['coverage'] else "🔴")
        print(f"    {icon} {tk['id']}: Top-K {tk['coverage']*100:.0f}% → Graph {ga['coverage']*100:.0f}% (noise: {tk['noise']}→{ga['noise']})")

    print("  ✅ PASS\n")

    return topk_results, greedy_results


if __name__ == '__main__':
    print("\n🦍🧠 Kong Graph — Full Test Suite\n")

    g = test_graph_construction()
    test_neighborhood_search(g)
    test_subgraph_optimizer()
    test_full_pipeline()
    topk, graph_results = test_memcombine_benchmark()
    ...
    print(f"  Graph Coverage:  {graph_results['avg_coverage']*100:.1f}%")
    print(f"  Advantage:       {(graph_results['avg_coverage']-topk['avg_coverage'])*100:+.1f}%")
