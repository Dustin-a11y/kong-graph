"""
Ollama + Kong Graph Memory Middleware
===============================
...
injects recalled Kong Graph memories into the system prompt and stores every
...
Architecture:
    ┌──────────┐      ┌─────────────────┐      ┌──────────┐
    │  Client   │─────▶│ Kong Graph Middleware  │─────▶│  Ollama   │
...
Flow per request:
  ...
  2. recall() from Kong Graph — get K most relevant past memories
  ...
  6. store() the user message and assistant reply into Kong Graph
...
# ---------------------------------------------------------------------------
# Kong Graph import — graceful fallback
# ---------------------------------------------------------------------------
try:
    ...
    QMG_AVAILABLE = False
    print("[WARNING] kong-graph not installed.")
    print("[WARNING] Install with: pip install kong-graph")
...
# ---------------------------------------------------------------------------
# Helper: inject Kong Graph memories into the message list
# ---------------------------------------------------------------------------
def augment_messages(messages: list[dict], k: int = MEMORY_K) -> list[dict]:
    ...
    recall relevant Kong Graph memories, and prepend them as a system message.
...
def chat():
    ...
    # ── Augment with Kong Graph memories ──────────────────────────────────
