"""
llama-cpp + Kong Graph Memory Integration
===================================
...
Integration pattern:
    ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
    │  User Input   │────▶│ Kong Graph recall() │────▶│  System      │
    │               │     │ (K relevant  │     │  Prompt      │
    └───────────────┘     │  memories)   │     └──────┬───────┘
                          └──────────────┘            │
                                                      ▼
    ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
    │  Return       │◀────│ Kong Graph store()  │◀────│ llama-cpp    │
    │  Response     │     │ (persist     │     │ generate()   │
    │               │     │  exchange)   │     │              │
    └───────────────┘     └──────────────┘     └──────────────┘

Each turn:
  1. Recall the K most relevant past memories using embedding + graph traversal + greedy selection
...
# ---------------------------------------------------------------------------
# Kong Graph import — gracefully handle missing package
# ---------------------------------------------------------------------------
try:
    ...
    QMG_AVAILABLE = False
    print("[WARNING] kong-graph not installed. Install with: pip install kong-graph")
...
# ---------------------------------------------------------------------------
# Chat session with Kong Graph memory
# ---------------------------------------------------------------------------

class QMGMemoryChat:
    """
    A chat session that uses Kong Graph as its persistent memory backend.
    ...
    mock : bool
        ...
        Useful for testing the Kong Graph integration without a GPU / large download.
    memory_k : int
    ...
    def chat(self, user_message: str) -> str:
        ...
        1. Recall relevant past memories from Kong Graph
        2. Build a system prompt that includes those memories as context
        3. Generate the assistant's response
        4. Store the full exchange back into Kong Graph for future recall
        """
        # ── Step 1: Recall ──────────────────────────────────────────
        try:
...
def run_demo():
    ...
    print("=" * 60)
    print("  Kong Graph + llama-cpp Memory Demo (mock mode)")
    print("=" * 60)
    ...
    print(f"\n{'─' * 40}")
    print("Demo complete. All exchanges stored in Kong Graph memory graph.")
...
def run_interactive():
    ...
    print("=" * 60)
    print("  Kong Graph Memory Chat — type 'quit' to exit, 'memories' to recall")
    print("=" * 60)
