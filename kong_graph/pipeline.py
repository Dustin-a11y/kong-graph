...
This is what users interact with:
  ...
  recall(query, K) → graph neighborhood → greedy subgraph → optimal memories
...
def recall(
    ...
    Args:
        ...
        max_candidates: Max nodes for subgraph selection
        method: "greedy" (default) or "synergy"
    ...
    # Phase 2: Synergy rerank or greedy subgraph
    if method == "synergy":
        try:
            ...
            print(f"WARNING: Synergy selection failed ({e}), falling back to greedy")

    # Phase 3: Greedy subgraph optimization
    ...
    try:
        ...
        print(f"WARNING: Subgraph optimization failed ({e}), using fallback")
        result = {
            "selection": list(range(min(K, len(candidate_ids)))),
            "score": float(sum(candidate_scores[:K])),
            "greedy": {"selection": list(range(K)), "score": float(sum(candidate_scores[:K]))},
            "optimal": {"selection": list(range(K)), "score": float(sum(candidate_scores[:K]))},
            "method": "fallback",
        }
    ...
    return {
        "ok": True,
        "memories": selected_memories,
        "method": "greedy_subgraph",
        "candidates": len(candidate_ids),
        "K": K,
        "subgraph_score": result["score"],
        "tier_counts": {
